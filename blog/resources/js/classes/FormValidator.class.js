class FormValidator
{
    ...
    /**
     * Vérifie les types de données à saisir
     */
    checkDataTypes() {
        var errors;
    ...
    /**
     * Vérifie la longueur minimale des données saisies
     */
    checkMinimumLength() {
        var errors;
    ...
    /**
     * Vérifie les champs obligatoires (required)
     */
    checkRequiredFields() {
        var errors;
        ...
        this.$form.find('[data-required]').each(function()
        {
            ...
            * elle représente tous les objets DOM sélectionnés.
            *
            * Pour notre cas elle représente donc tour à tour chaque champ du
            * formulaire trouvé avec la méthode jQuery find().
            */
    ...
    /** 
     * Lance les vérifications lors du submit
     * @param {event} event
     */
    onSubmitForm(event) {
        var $errorList;
        ...
        if(this.totalErrors.length > 0)
        {
            ...
            this.$totalErrorCount.html(this.totalErrors.length);
    ...
    /**
     * Lancer la validation d'un formulaire à l'aide d'un écouteur d'événement
     */
    run() {
